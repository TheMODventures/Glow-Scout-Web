...
const BusinessSettings = () => {
  ...
  useEffect(() => {
    if (currentUser) {
      setLinks({
        link1: currentUser.links?.find(link => link.platform === "instagram")?.url || "",
        link2: currentUser.links?.find(link => link.platform === "facebook")?.url || "",
        link3: currentUser.links?.find(link => link.platform === "snapchat")?.url || "",
      });
    }
  }, [currentUser]);
...
const handleFileChange = (e, id) => {
  ...
  async function handleSubmit(data) {
    ...
    Object.keys(timings).forEach(day => {
        const formattedStartTime = formatTime(timings[day].openTime);
        const formattedEndTime = formatTime(timings[day].closeTime);

        formattedTimings[day] = {
            ON: timings[day].openTime !== "Closed",
    ...
    Object.keys(formattedTimings).forEach(day => {
        ...
        formData.append(`businessTiming.${day}.startTime`, formattedTimings[day].startTime);
        formData.append(`businessTiming.${day}.endTime`, formattedTimings[day].endTime);
    });
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} encType="multipart/form-data">
          ...
          <div className="col-span-1 pr-0 lg:pr-5 lg:border-r-2 border-darkMahron">
            ...
            <div className="flex flex-col mt-4">
              <div className="flex justify-between items-center text-darkMahron mb-4">
                <h2 className="text-xl font-semibold w-1/3">
                  Timings
                </h2>
                <p className="text-sm w-1/3 text-center text-gray-300">Open</p>
                <p className="text-sm w-1/3 text-center text-gray-300">Close</p>
              </div>
        ...
        <div className="flex flex-col sm:flex-row justify-center mt-8 space-y-4 sm:space-y-0 sm:space-x-4">
          <button type="button" onClick={(()=>window.location.reload())} className="px-6 py-2 border-2 border-darkMahron text-darkMahron rounded-full">
            Discard changes
          </button>
          <button
            type="submit"
